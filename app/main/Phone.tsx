...
const dataColors = {
  ...
  'Frontend Mentor': ['/assets/images/icon-frontend-mentorWhite.svg', 'bg-[#FFFFFF]'],
        ...
        <div className="flex h-[300px] w-full flex-col">
          <ul className="flex h-fit w-full flex-col gap-[20px] overflow-auto bg-white">
            {socialInfo?.map((item, index) => (
              ...
              >
                <div className="flex h-full w-full items-center fill-white *:fill-white *:text-white">
                  <Image
                    color="inherit"
                    style={{ fill: 'white', color: 'inherit' }}
                    src={dataColors[item as keyof typeof dataColors][0]}
                    alt={item}
                    width={10}
                    height={10}
                    className="fill-initial ml-[16px] mr-[16px] h-[16px] w-[16px] fill-inherit text-inherit"
                  />
